<?php
	...
	$forceFresh = mysql_real_escape_string($_GET['forceFresh']);
	
	/*	Check Local Database for Schedule if User is not forcing to get Fresh Data*/
	if(!$forceFresh)
	{
		$jsonResponse = json_decode(file_get_contents('https://studentweb.ok.ubc.ca/31317092/project/phpDataAccessors/getStoredSchedule.php?ubcid='.$ubcid), TRUE);
	}
	...
	if($forceFresh || count($jsonResponse)==0)
	{
		...
		$freshResponse = json_decode(file_get_contents('https://studentweb.ok.ubc.ca/31317092/project/phpDataAccessors/getFreshSchedule.php?ubcid='.$ubcid), TRUE);
